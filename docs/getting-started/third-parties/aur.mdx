...
`http://[address]:5055` and completing the setup steps. Direct HTTP login works
by default with a warning. Use the first-run transport choice for [built-in
HTTPS](/using-seerr/advanced/built-in-tls) or an HTTPS reverse proxy for
encrypted sessions; set `SEERR_ALLOW_HTTP_AUTH=false` to require HTTPS.
:::
