...
:::note
Images built by a fork

A fork of this repository publishes its own image to that repository's GitHub Packages registry on every update to `main`, tagged `latest`, the branch name, and the commit SHA:

```bash
docker pull ghcr.io/<owner>/<repo>:main
```

These builds are `linux/amd64` only. Unlike the official images above they are neither signed nor accompanied by an SBOM, so the verification guide does not apply to them. Prefer an official image unless you specifically want the fork's changes.
:::
